...
static int tcp_ack(struct sock *sk, const struct sk_buff *skb, int flag)
{
	...
	if (net->ipv4.sysctl_tcp_vtcp == 1) {
		 ...
		 */
		if (th->ece && !th->syn && (tp->ecn_flags & TCP_ECN_OK)
				&& (!tp->vtcp_state.last_cwnd_red_ts || (before(ack, tp->vtcp_state.last_cwnd_red_ts - (tp->vtcp_state.last_window *3) / 4) && after(ack, tp->vtcp_state.last_cwnd_red_ts - (tp->vtcp_state.last_window * 7)/8))
